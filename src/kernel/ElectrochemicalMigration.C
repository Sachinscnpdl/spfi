 ...
 */
registerMooseObject("spfiApp", ElectrochemicalMigration);


// Definition of validParams() without template specialization
InputParameters ElectrochemicalMigration::validParams()
{
  InputParameters params = Kernel::validParams();
  params.addClassDescription("Add in ElectrochemicalMigration,subroutine for electrochemicalMigration equation -[delQdel(cv)+delQNdel(cp)]");
  ...
  params.addRequiredCoupledVar("c_pot", "applied potential as coupled variable");
  params.addRequiredCoupledVar("c_eta", "eta as coupled variable");
  return params;
}
...
Real ElectrochemicalMigration::computeQpResidual()
{
  return _Q[_qp] * _grad_c_eta[_qp] * _grad_test[_i][_qp] + _QM[_qp] * _grad_c_pot[_qp] * _grad_test[_i][_qp];
}

Real ElectrochemicalMigration::computeQpJacobian()
{
  return _dQ[_qp] * _grad_c_eta[_qp] * _grad_test[_i][_qp] * _phi[_j][_qp] + _dQM[_qp] * _grad_c_pot[_qp] * _grad_test[_i][_qp] * _phi[_j][_qp];
}

Real ElectrochemicalMigration::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _c_pot_var)
    return _QM[_qp] * _grad_phi[_j][_qp] * _grad_test[_i][_qp] + _dQp[_qp] * _grad_c_eta[_qp] * _grad_test[_i][_qp] * _phi[_j][_qp] + _dQMp[_qp] * _grad_c_pot[_qp] * _grad_test[_i][_qp] * _phi[_j][_qp];
  else if (jvar == _c_eta_var)
    return _Q[_qp] * _grad_phi[_j][_qp] * _grad_test[_i][_qp] + _dQv[_qp] * _grad_c_eta[_qp] * _grad_test[_i][_qp] * _phi[_j][_qp] + _dQMv[_qp] * _grad_c_pot[_qp] * _grad_test[_i][_qp] * _phi[_j][_qp];
  else
    return 0;
}
