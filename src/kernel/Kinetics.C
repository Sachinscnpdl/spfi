...
registerMooseObject("spfiApp", Kinetics);

InputParameters Kinetics::validParams()
{
  InputParameters params = Kernel::validParams();
  params.addClassDescription("Add in Kinetics");
  params.addRequiredCoupledVar("cp", "Coupled variable cp.");
  params.addRequiredCoupledVar("cv", "Coupled variable cv.");
...
Real Kinetics::computeQpResidual()
{
  return _F[_qp] * _test[_i][_qp];
}

Real Kinetics::computeQpJacobian()
{
  return _dFe[_qp] * _phi[_j][_qp] * _test[_i][_qp];
}

Real Kinetics::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _cp_var)
